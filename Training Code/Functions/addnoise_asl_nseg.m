function [noisy,noise_segment] = addnoise_asl_nseg(clean,srate,nbits,noise,srate1,nbits1, snr) 
% ----------------------------------------------------------------------
...
%
%  Example call:
%       addnoise_asl(clean,srate,nbits,noise,srate1,nbits1, outfile(.wav file), snrindb);
%
%  
%  References:
...
if ( (max( noisy)>= 1) | (min( noisy)< -1))
    while (max( noisy)>= 1) | (min( noisy)< -1)
        noise_segment = 0.99.*noise_segment;
        noisy = x + noise_segment;
    end
